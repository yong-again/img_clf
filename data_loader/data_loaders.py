...
class CarAugMixImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_name = row['image_name']
        folder_name = row['folder_name']
        img_full_path = os.path.join(self.data_dir, 'train', folder_name, image_name)
        image = Image.open(img_full_path).convert('RGB')
...
class CarAugMixImageDataLoader(BaseDataLoader):
    def __init__(self, data_dir, csv_file, batch_size, shuffle=True, validation_split=0.0, num_workers=2, return_onehot=False):
        trsfm = train_transform_augmix()  # AugMix 기반 transform
        self.dataset = CarAugMixImageDataset(data_dir=data_dir, csv_file=csv_file, return_onehot=return_onehot, transform=trsfm)
        super().__init__(self.dataset, batch_size, shuffle, validation_split, num_workers)
